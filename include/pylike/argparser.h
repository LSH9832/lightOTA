/*
 * copy and modified from https://github.com/0382/util/blob/main/cpp/argparse/argparse.hpp
 */
#pragma once
...
class ArgumentParser
{
  ...
  public:
    ...
    ArgumentParser &add_sc_option(std::string sname, std::string lname, std::string help, std::function<void(void)> callback)
    {
    ...
    template <typename T>
    ArgumentParser &add_option(std::string sname, std::string lname, std::string help, T &&default_value)
    {
        if (type_string<T>() == "null")
        {
            // std::cerr << "(build error) unsupport type for option: " << typeid(T).name() << std::endl;
            std::cerr << "(build error) unsupport type for " << lname << std::endl;
            std::exit(-1);
        }
        check_add_option_lname(lname);
        if (sname != "")
    ...
    ArgumentParser &add_option(std::string sname, std::string lname, std::string help)
    {
        check_add_option_lname(lname);
        if (sname != "")
        {
    ...
    template <typename T>
    ArgumentParser &add_argument(std::string name, std::string help)
    {
    ...
    template <typename T>
    ArgumentParser &add_named_argument(std::string name, std::string help)
    {
        check_add_argument_name<T>(name);
  ...
  private:
    ...
    void check_add_argument_name(const std::string &key) const
    {
        if (type_string<T>() == "null")
        {
            ...
            std::cerr << "(build error) argument type is not supported: " << key << std::endl;
            std::exit(-1);
        }
