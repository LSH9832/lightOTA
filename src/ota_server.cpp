...
|_____|_|\__, |_| |_|\__|\___/ |_/_/   \_\
    ...
    void run()
    {
        ...
        domains = getValueFromEnv("OTA_DOMAINS", domains);
        domain = getValueFromEnv("OTA_DOMAIN", domain);
        std::vector<std::string> domain_routes = {""};
        if (!domain.empty())
        {
            domain = "/" + domain;
            domain_routes.push_back(domain);
        }
        ...
        for (std::string d: domain_routes)
        {
            app->route2(prefix + d + "/api/getOTAFileList", [&](const RequestData& req) {
                if (req.isFromData)
                {
                    req.acceptForm();
                }
            ...
            app->route2(prefix + d + "/api/getSoftwareList", [&](const RequestData& req) {
                if (req.isFromData)
                {
                    req.acceptForm();
                }
            ...
            app->route2(prefix + d + "/api/update", [&](const RequestData& req) {
                if (req.isFromData)
                {
                    req.acceptForm();
                }
            ...
            app->route2(prefix + d + "/api/info", [&](const RequestData& req) {
                if (req.isFromData)
                {
                    req.acceptForm();
                }
            ...
            app->route2(prefix + d + "/api/deleteFile", [&](const RequestData& req) {
                if (req.isFromData)
                {
                    req.acceptForm();
                }
            ...
            app->route2(prefix + d + "/api/uploadFile", [&](const RequestData& req) {
                flaskcpp::JsonGenerator js;
                js.add("success", true);
                bool has_file=false;
                int ret_code = 200;
            ...
            app->route2(prefix + d + "/api/get_html_path", [&](const RequestData& req) {
